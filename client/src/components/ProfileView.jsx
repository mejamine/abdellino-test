...
export default function ProfileView() {
    ...
    useEffect(() => {
        const loadCommandeP=async()=>{
            try {
                const response = await fetch(`http://localhost:4000/abdellino/commandes/user/${userId}`);
                if (response.ok) {
        ...
        const loadCommandeM = async()=>{
            try {
                const response = await fetch(`http://localhost:4000/abdellino/commandesm/user/${userId}`);
                if (response.ok) {
                  const data = await response.json();
                  setCommandesM(data);
                } else {
